#!/bin/bash
###############################################################################
# src/db.sh — All SQLite database interactions
#
# Single DB: output/db/db.sqlite  ($DB_FILE)
# Stores both article collection data (journal, article, author, repositories,
# notebook_mentions) and pipeline execution results (repo_targets, notebooks,
# repository_runs, notebook_executions, notebook_reproducibility_metrics).
###############################################################################
...
get_repo_id_from_db() {
    ...
    sqlite3 "$DB_FILE" "SELECT id FROM repo_targets WHERE repository = '$repo_path' LIMIT 1;"
}
