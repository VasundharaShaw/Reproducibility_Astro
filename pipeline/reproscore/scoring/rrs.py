"""
pipeline/reproscore/scoring/rrs.py
===================================
...
One change from upstream: notebook_paths imported from local utils package.
"""
...
from ..utils.notebook_paths import is_excluded_notebook
...
class _SubMetricDetectors:
    ...
    def dep_pinning(repo: Path) -> SubMetricEvidence:
        ...
        for fname in lockfiles:
            if (repo / fname).exists():
                return SubMetricEvidence(
                    ...
                    file_path=fname,
                    deduction_reason=f"Lockfile found: {fname}",
                )
        ...
        return SubMetricEvidence(
            ...
            fix_suggestion="Add requirements.txt with pinned versions or a lockfile.",
        )
    ...
    def container_spec(repo: Path) -> SubMetricEvidence:
        ...
        for pat in glob_patterns:
            ...
            if has_from and has_run:
                return SubMetricEvidence(
                    ...
                    fix_suggestion="Pin the base image tag for full reproducibility.",
                )
            if has_from:
                return SubMetricEvidence(
        ...
        return SubMetricEvidence(
            ...
            file_path=None,
            deduction_reason="No container specification found",
    ...
    def env_bootstrap(repo: Path) -> SubMetricEvidence:
        ...
        return SubMetricEvidence(
            ...
            fix_suggestion="Add an install.sh or Makefile setup target.",
        )
    ...
    def python_version_declared(repo: Path) -> SubMetricEvidence:
        ...
        return SubMetricEvidence(
            ...
            fix_suggestion="Add a .python-version file or python_requires in pyproject.toml.",
        )
    ...
    def data_description(repo: Path) -> SubMetricEvidence:
        ...
        return SubMetricEvidence(
            ...
            fix_suggestion="Add a DATA.md or '## Data' README section.",
        )
    ...
    def data_pointer(repo: Path) -> SubMetricEvidence:
        ...
        for doc in ["DATA.md", "data/README.md"]:
            fp = repo / doc
            if fp.exists():
    ...
    def workflow_orchestration(repo: Path) -> SubMetricEvidence:
        ...
        return SubMetricEvidence(
            ...
            fix_suggestion="Add a Snakefile, Makefile, or Nextflow pipeline.",
        )
    ...
    def data_acquisition_script(repo: Path) -> SubMetricEvidence:
        ...
        if dvc_files:
            return SubMetricEvidence(
                ...
                deduction_reason="DVC data tracking file found",
            )
        ...
        check.extend(f for f in sorted(repo.glob("**/*.py"))[:30] if not is_excluded_notebook(f))

        for f in check:
            if download_re.search(f.read_text(errors="replace")):
                return SubMetricEvidence(
                    metric_id="data_acquisition_script", category="A", raw_score=100,
        ...
        for nb in [f for f in sorted(repo.glob("**/*.ipynb")) if not is_excluded_notebook(f)][:20]:
        ...
        return SubMetricEvidence(
            ...
            fix_suggestion="Add a download script or DVC tracking.",
        )
    ...
    def doc_structure(repo: Path) -> SubMetricEvidence:
        ...
        if not readme:
            return SubMetricEvidence(
                ...
                fix_suggestion="Add a README.md with Installation, Usage, Expected Output, Requirements sections.",
            )
        content = readme.read_text(errors="replace")
    ...
    def execution_entry_point(repo: Path) -> SubMetricEvidence:
        ...
        return SubMetricEvidence(
            ...
            fix_suggestion="Add a run.sh or '## How to Run' README section.",
        )

    @staticmethod
    def reuse_metadata(repo: Path) -> SubMetricEvidence:
        found = []
        for fname in ["LICENSE", "LICENSE.md", "LICENSE.txt", "LICENSE.rst",
        ...
        return SubMetricEvidence(
            ...
            fix_suggestion="Add a LICENSE file and optionally a CITATION.cff.",
        )
    ...
    def no_absolute_paths(repo: Path) -> SubMetricEvidence:
        ...
        if total_files == 0:
            return SubMetricEvidence(
                ...
                file_path=None, deduction_reason="No source files found",
            )
    ...
    def import_resolvability(repo: Path) -> SubMetricEvidence:
        ...
        pkg_name_re = re.compile(
            r'(?:^|[\s\-,\[\(])([a-zA-Z][a-zA-Z0-9_\-\.]{1,})\s*(?:[>=<!,\]\)]|$)', re.M
        )
        ...
        if not declared:
            return SubMetricEvidence(
                ...
                file_path=None,
                deduction_reason="No dependency specification files found",
                fix_suggestion="Add requirements.txt or environment.yml.",
            )
        ...
        if not third_party:
            return SubMetricEvidence(
                ...
                file_path=None, deduction_reason="No third-party imports detected",
            )
        ...
        return SubMetricEvidence(
            ...
            file_path=None,
            deduction_reason=(
                f"{len(third_party) - len(undeclared)}/{len(third_party)} imports declared"
                + (f"; undeclared: {example}" if undeclared else "")
            ),
            fix_suggestion=f"Add to requirements.txt: {example}" if undeclared else "",
        )
    ...
    def no_hardcoded_credentials(repo: Path) -> SubMetricEvidence:
        ...
        return SubMetricEvidence(
            ...
            fix_suggestion="Replace hardcoded credentials with environment variables.",
        )
    ...
    def silent_failure_masking(repo: Path) -> SubMetricEvidence:
        ...
        if total_files == 0:
            return SubMetricEvidence(
                ...
                file_path=None, deduction_reason="No Python source files found",
            )
        ...
        return SubMetricEvidence(
            ...
            fix_suggestion="Replace bare except:pass with specific exception handling.",
        )
    ...
    def seed_management(repo: Path) -> Tuple[SubMetricEvidence, float]:
        ...
        ev = SubMetricEvidence(
            ...
            deduction_reason=(
                f"Deterministic execution: {len(f_seed)}/{len(f_rand)} "
                f"stochastic files have seed-setting calls (σ={sigma:.2f})"
            ),
            fix_suggestion=(
                f"Add seed-setting in: {', '.join(f_rand[:3])}." if sigma < 1.0 and f_rand else ""
            ),
        )
        return ev, sigma
    ...
    def notebook_exec_order(repo: Path) -> SubMetricEvidence:
        ...
        if not nb_files:
            return SubMetricEvidence(
                ...
                file_path=None, deduction_reason="No notebooks found — metric not applicable",
            )
        ...
        if n_total == 0:
            return SubMetricEvidence(
                metric_id="notebook_exec_order", category="S", raw_score=100,
                file_path=None, deduction_reason="No executed notebooks found",
            )
        ...
        return SubMetricEvidence(
            ...
            fix_suggestion=(
                "Re-run notebooks top-to-bottom before committing." if score < 100 else ""
            ),
        )
    ...
    def test_file_presence(repo: Path) -> SubMetricEvidence:
        ...
        return SubMetricEvidence(
            ...
            fix_suggestion="Add test files (test_*.py) using pytest." if count == 0 else "",
        )
    ...
    def expected_outputs(repo: Path) -> SubMetricEvidence:
        ...
        return SubMetricEvidence(
            ...
            fix_suggestion="Add a results/ directory with reference outputs.",
        )
        ...
        for ci in ci_indicators:
            if (repo / ci).exists():
                return SubMetricEvidence(
                    ...
                    file_path=ci, deduction_reason=f"CI configuration: {ci}",
                )
        return SubMetricEvidence(
            metric_id="ci_presence", category="S", raw_score=0,
            file_path=None,
            deduction_reason="No CI configuration found",
            fix_suggestion="Add a GitHub Actions workflow (.github/workflows/).",
        )
    ...
    def config_externalised(repo: Path) -> SubMetricEvidence:
        ...
        for fname in config_files:
            if (repo / fname).exists():
                return SubMetricEvidence(
                    ...
                    file_path=fname, deduction_reason=f"External config file: {fname}",
                )
        ...
        return SubMetricEvidence(
            ...
            fix_suggestion="Store parameters in config.yaml or use argparse.",
        )
    ...
    def hardware_requirements(repo: Path) -> SubMetricEvidence:
        ...
        return SubMetricEvidence(
            ...
            fix_suggestion="Declare GPU/CUDA requirements in README.",
        )
...
class RRSScorer:
    ...
    def score(self, repo_path: str | Path) -> RRSResult:
        ...
        category_scores = {
            "E": CategoryScore("Environment Specification", "E", raw_E, gated_E * 100, sub_E),
            "A": CategoryScore("Data Accessibility",        "A", raw_A, gated_A * 100, sub_A),
            "D": CategoryScore("Documentation",             "D", raw_D, gated_D * 100, sub_D),
            "C": CategoryScore("Code Portability",          "C", raw_C, gated_C * 100, sub_C),
            "S": CategoryScore("Reproducibility Signals",   "S", raw_S, gated_S * 100, sub_S),
        }
